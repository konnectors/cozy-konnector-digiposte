const getFileName = (doc) => {
  ...
  return doc.filename.split('.').reduce((accumulator, fragment, index, fragments) => {
    const hasNoFileExtension = fragments.length === 1
    if (hasNoFileExtension) {
      return `${fragment}_${creationDate.toLocaleDateString()}`
    }

    const isFileExtension = index === fragments.length - 1
    if (isFileExtension) {
      return `${accumulator}_${creationDate.toLocaleDateString()}.${fragment}`
    }
